...
export async function matchAnime(url, req) {
  try {
    ...
    // 首先尝试匹配电视剧格式 SxxExx
    const tvRegex = /^(.+?)[.\s]+S(\d+)E(\d+)/i;
    const tvMatch = cleanFileName.match(tvRegex);
    
    // 然后尝试匹配电影格式（包含年份）
    const movieRegex = /^(.+?)[.\s]+(\d{4})[.\s]/;
    const movieMatch = cleanFileName.match(movieRegex);
    
    let title, season = null, episode = null, year = null;
    ...
    // 构建搜索关键词，如果有年份信息则添加到关键词中以提高匹配准确性
    let searchKeyword = title;
    if (year) {
      searchKeyword = `${title} ${year}`;
    }
    let originSearchUrl = new URL(req.url.replace("/match", `/search/anime?keyword=${searchKeyword}`));
